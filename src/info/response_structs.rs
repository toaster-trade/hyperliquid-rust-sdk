...
pub struct AssetStreamingOiCapPair(pub String, pub String);

#[derive(Debug, Deserialize)]
pub struct SubDeployerPair(pub String, pub Vec<Address>);

impl From<AssetStreamingOiCapPair> for AssetStreamingOiCap {
    fn from(pair: AssetStreamingOiCapPair) -> Self {
        Self {
            asset: pair.0,
            streaming_oi_cap: pair.1,
        }
    }
}
...
pub struct PerpDexInfo {
    ...
    #[serde(from = "Vec<AssetStreamingOiCapPair>")]
    pub asset_to_streaming_oi_cap: Vec<AssetStreamingOiCap>,
    #[serde(from = "Vec<SubDeployerPair>")]
...
#[derive(Deserialize, Debug)]
#[serde(from = "AssetStreamingOiCapPair")]
pub struct AssetStreamingOiCap {
    pub asset: String,
    pub streaming_oi_cap: String,
}
...
impl From<SubDeployerPair> for SubDeployer {
    fn from(pair: SubDeployerPair) -> Self {
        Self {
            action: pair.0,
            addresses: pair.1,
        }
